...
$.extend($.validator, {
	...
	messages: {
		...
		channelID: $.validator.format("请输入{0}至{1}个英文数字或下划线。"),
		channelName:$.validator.format("请输入{0}至{1}个个汉字。")
	},
	...
	methods: {
		...
		//限定数量的英文数字或下划线
		channelID:function(value,element,param){
			var length = $.isArray( value ) ? value.length : this.getLength($.trim(value), element);
			return /^[0-9a-zA-Z_]{1,}$/.test(value) && ( length >= param[0] && length <= param[1] ) ;
		},

		// 限定数量的汉字
		channelName:function(value,element,param){
			var length = $.isArray( value ) ? value.length : this.getLength($.trim(value), element);
			return /^[\u4e00-\u9fa5]{0,}$/.test(value) && ( length >= param[0] && length <= param[1] ) ;
		}
